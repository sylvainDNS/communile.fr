# Implementation Plan: Page « La Sibra »
...
Créer la page dédiée à la microbrasserie « La Sibra » (anciennement Les Bières de Charlotte, `/la-sibra`) sur le patron éprouvé des 5 pages de lieux existantes : un dossier `src/features/sibra/` (sections + images), un thème de couleurs imposé par la charte fournie le 2026-09-18 (vert olive `#7A9300`, rose `#DC5B87`, orange `#FEA300` + accents dérivés) déclaré dans `global.css` et le type `theme` de `main.astro`, une décoration hero « bulles » et un ornement « trois disques » (SVG `currentColor`, proposés dans `specs/001-page-brasserie/assets/`), une page `src/pages/la-sibra.astro` avec métadonnées SEO et JSON-LD `Brewery`, et l'intégration aux points de découverte (header, footer, section lieux de l'accueil). Le sitemap, exigé par FR-010 et la constitution mais absent du site, est ajouté sous forme d'endpoint statique. Modification transverse : le paragraphe « dernier lieu né » de la page À La Carte Postale devient « quatrième restaurant » (FR-015). Aucune nouvelle dépendance. Nom du lieu tranché le 2026-09-18 : « La Sibra » (research R11).
...
**Storage**: N/A — contenu rédactionnel codé en dur dans les sections `.astro`, comme les 5 pages existantes. Images dans `src/features/sibra/images/` (webp).
...
**Constraints**: WCAG 2.1 AA (couleurs de charte non modifiables → règles d'usage texte blanc/sombre du contrat thème) ; pas de JS ajouté hors composants existants ; photos sources portrait 3472×4624 (~3,5 Mo) à recadrer/convertir ; coordonnées (FR-011) à faire confirmer par la coopérative avant mise en ligne.
...
│   └── theme.md         # Tokens de couleurs du thème brasserie (charte 2026-09-18)
├── assets/              # SVG proposés (hero « bulles », ornement « trois disques », repli)
...
src/
├── pages/
│   ├── la-sibra.astro                       # NOUVEAU — page du lieu (layout + sections + JSON-LD Brewery)
│   └── sitemap.xml.ts                       # NOUVEAU — endpoint sitemap (toutes les pages publiques)
├── features/
│   ├── sibra/                               # NOUVEAU — feature du lieu
│   │   ├── images/
│   │   │   ├── sibra-hero-decoration.svg        # depuis specs/…/assets/ (T007)
│   │   │   ├── sibra-what-ornament.svg          # depuis specs/…/assets/ (T007)
│   │   │   ├── sibra-boutique.webp              # depuis brasserie-assets/1000016867.jpg
│   │   │   ├── sibra-brassage.webp              # depuis brasserie-assets/1000016868.jpg
│   │   │   └── sibra-bouteilles.webp            # depuis brasserie-assets/1000016869.jpg
│   │   └── sections/
│   │       ├── sibra-hero-section.astro
│   │       ├── sibra-what-section.astro
│   │       ├── sibra-boutique-section.astro
│   │       ├── sibra-bieres-section.astro
│   │       ├── sibra-tireuse-section.astro
│   │       ├── sibra-infos-section.astro
│   │       └── sibra-instagram-section.astro
│   └── home/images/
│       └── la-sibra-card.webp               # NOUVEAU — recadrage paysage pour la carte d'accueil
├── layouts/main.astro                        # MODIFIÉ — union `theme` + 'sibra'
├── styles/global.css                         # MODIFIÉ — couleurs brasserie + [data-theme='sibra']
├── components/
...
├── features/la-carte-postale/sections/la-carte-postale-what-section.astro  # MODIFIÉ — « quatrième restaurant » (FR-015)
└── utils/constants.ts                        # MODIFIÉ — PATH.LA_SIBRA

public/
└── robots.txt                                # NOUVEAU — référence le sitemap
```

**Structure Decision**: architecture par features du dépôt reproduite à l'identique : une feature `sibra` (sections + images), une page dans `src/pages/`, le thème dans `global.css`/`main.astro`, les intégrations dans les composants partagés. Le sitemap est le seul ajout transverse (exigé par FR-010, absent du site à ce jour).
